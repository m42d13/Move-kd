...
python -u -m deepspeed.launcher.launch \
    --world_info=eyJsb2NhbGhvc3QiOiBbMF19 \
    ...
    --bf16 True \
    --output_dir "${OUTPUT_DIR:-./checkpoints/finetune/full-adapter-move-kd-7b-v1.1-metrics}" \
    --num_train_epochs 1 \
    ...
    --run_name "${RUN_NAME:-move-kd-v1.1-metrics}" \
    --finetune False \
    --tune_encoder True \
    --tune_all False \
    --moe_encoder True \
