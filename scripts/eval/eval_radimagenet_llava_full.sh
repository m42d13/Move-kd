...
LLAVA_ROOT="/home/baduc/LLaVA"
PYTHON="/home/baduc/miniconda3/envs/llava/bin/python"

MODEL_PATH="${MODEL_PATH:-${ROOT}/checkpoints/llava-medVQA-7b-stage2-lora-fixed}"
MODEL_BASE="${MODEL_BASE:-lmsys/vicuna-7b-v1.5}"
DATA_ROOT="${DATA_ROOT:-${ROOT}/playground/data/radimagenet_test}"
OUT_DIR="${OUT_DIR:-${DATA_ROOT}/eval_full}"
export DATA_ROOT OUT_DIR

QUESTIONS="${OUT_DIR}/questions_full.jsonl"
ANSWER_KEY="${OUT_DIR}/answer_key_full.jsonl"
ANSWERS="${OUT_DIR}/answers_full_llava.jsonl"
SCORED="${OUT_DIR}/scored_full_llava.jsonl"
METRICS="${OUT_DIR}/metrics_full_llava.json"
...
root = Path(os.environ.get("DATA_ROOT", "/home/baduc/MoVE-KD/playground/data/radimagenet_test"))
out_dir = Path(os.environ.get("OUT_DIR", root / "eval_full"))
...
echo "[2/3] Running LLaVA generation..."
(
    cd "${LLAVA_ROOT}"
    PYTHONPATH="${LLAVA_ROOT}" \
    HF_HUB_OFFLINE=1 \
    TRANSFORMERS_OFFLINE=1 \
    CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" \
    TRITON_CACHE_DIR="${TRITON_CACHE_DIR:-/tmp/triton-cache-llava}" \
...
out_dir = Path(os.environ.get("OUT_DIR", "/home/baduc/MoVE-KD/playground/data/radimagenet_test/eval_full"))
answers_path = out_dir / "answers_full_llava.jsonl"
key_path = out_dir / "answer_key_full.jsonl"
metrics_path = out_dir / "metrics_full_llava.json"
scored_path = out_dir / "scored_full_llava.jsonl"
